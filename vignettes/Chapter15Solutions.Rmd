...
`To calculate a predictive likelihood on test data, we need to decide how many rows
should be assigned to the training set, and assign them in such a way that the training
and test sets are (approximately) independent. Notice that because Lena collected the
data in a paired design, with two sampling times at each station, random Stations need
to be chosen as the test sample, not random rows of data. The pair of rows in the dataset
corresponding to each station are jointly allocated to test or training data at random,
giving independent training and test sets if fish communities at different stations are
independent. Half of stations will be assigned to test data, and half to training.
```{r code15.1}
library(ecostats)
data(windFarms)
...
```{r ex15.3val}
windFt_Train=manyglm(windMVnotRare[isTest==FALSE,]~Year+Zone,
      data=windFarms$X[isTest==FALSE,],family="poisson")
windFt_Int_Train=manyglm(windMVnotRare[isTest==FALSE,]~Year*Zone,
      data=windFarms$X[isTest==FALSE,],family="poisson")
prWind_Test = predict(windFt_Train,newdata=windFarms$X[isTest,], type="response")
prWind_Int_Test = predict(windFt_Int_Train, newdata=windFarms$X[isTest,], type="response")
predLogL = dpois(windMVnotRare[isTest,],lambda=prWind_Test,log=TRUE)
predLogL_Int = dpois(windMVnotRare[isTest,],lambda=prWind_Int_Test,log=TRUE)
c(sum(predLogL),sum(predLogL_Int))
```
...
Without the rare species, the results are more reliable, in relative terms (the difference in predictive likelihoods is typically 5-10) and in absolute terms (the predictive likelihood is usually in the mid -700's). With rare species, it bounces around a lot more, with the smaller model (main effects only) sometimes having better predictive performance. This is likely because rare species with all absences in one treatment combination can blow up the predictive likelihood (if they are predicted to have a mean of zero, but have some non-zero cases in test data).
...
```{r code15.5, fig.width=6, fig.height=4}
library(VGAM)
...
```{r code15.6, fig.width=8, fig.height=8}
library(mvabund)
data(spider)
...
`To fit a mixed model, shrinking coefficients together across taxa, we will first put data
in long format, then use glmmTMB:
```{r code15.7}
