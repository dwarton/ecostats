...
for(iVar in 1:9)
  plotenvelope(lm(reveg$abund[,iVar]~fa_reveg$scores), which=2,main=print(names(reveg$abund)[iVar]))
for(iVar in 1:9)
  plotenvelope(lm(reveg$abund[,iVar]~fa_reveg$scores), which=3,main=print(names(reveg$abund)[iVar]))
```
...
Looks like the one latent variable model is the winner!

*Fit a Poisson model to the data and check assumptions. Are there any signs of overdispersion?*

I'll go with a one latent variable model, on account of this looking the best in the above.
```{r ex12.5, fig.width=9}
reveg_LVM1 = gllvm(reveg$abund, num.lv=1, family="poisson", trace=TRUE, jitter.var=0.2)
par(mfrow=c(1,3))
