...
Note that standard errors are larger and suddenly everything is non-signicant.
...
```{r seaweed anova}
 ...
 ft_habconf=lm(Total~Dist,data=seaweed)
 anova(ft_habconf)
```
...
```{r seaweed multcomp}
 ...
 compDist = glht(ft_habconf, linfct=contDist) # run multiple comparisions
 summary(compDist)
 plot(compDist)
```
...
```{r seaweed logre plot}
 plotenvelope(ft_habconf,which=1:3)
```

Notice that in the quantile plot that points get close to the lower bound in the middle of the plot and get near the top of it at large values -- this is mildly suggestive of skew. We similarly see an increaing trend in the scale-location plot, although it does stay within its simulatin bounds. Notice these are only subtle patterns and not particularly strong.

(As an aside, note there is no envelope visible on the residual vs fits plot -- this is because we are fitting an ANOVA model, which will have no mean trend on the res vs fits plot at all fitted values, that is, it is always a flat line at zero. Put another way, non-linearity isn't an assumption to worry about in ANOVA models.)
