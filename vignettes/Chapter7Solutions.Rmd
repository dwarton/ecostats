...
As before we don't see a lot of lines crossing over so expect some correlation. We also see a similar pattern with aphid counts reducing over time, but possibly being higher without bird exlcusion 2-5 weeks into the trial. 
...
# now try a model with a temporally structured random effect:
aphidw_CAR1 = update(aphidw_int2,correlation=corCAR1(,form=~Time|Plot))
...
```{r box711,fig.width=8,fig.height=8}
par(mfrow=c(2,2))
plot(pgls_egg)
...
```{r box711 otherplot,fig.width=6}
plot(res.pg)
```
