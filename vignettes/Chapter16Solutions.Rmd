...
for(iClust in 1:length(nClust))
{
  fti_Mix = species_mix(abundMat~treatment, data=revegDF, family="negative.binomial", nArchetypes=2, control=list(init_method='kmeans',ecm_refit=5, ecm_steps=2) )
  bics[iClust] = BIC(fti_Mix)
}
plot(bics~nClust, ylab="BIC", xlab="# archetypes")
```
...
ft_trait = gllvm(spider$abund, X, spider$trait, randomX=~X, family="negative.binomial")
logLik(ft_trait)
library(lattice)
...
#make predictions and plot:
newPreds = predict(ft_anttrait,newX=newEnv,newTR=newTraits,type="response")
...
```{r ex16.7}
ft_antspp = gllvm(antTraits$abund, envMat, family="negative.binomial", starting.val="zero")
ft_anttrait = gllvm(antTraits$abund, envMat, antTraits$traits, family="negative.binomial", starting.val="zero")
mainFormula=as.formula(paste("~", paste(colnames(envMat),collapse="+")))
ft_antmain = gllvm(antTraits$abund, envMat, antTraits$traits, family="negative.binomial", formula=mainFormula, starting.val="zero")
