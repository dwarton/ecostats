...
spiderX = data.frame(spider$x)
spider_glmInt = manyglm(spiderAbund~soil.dry,data=spiderX)
...
par(mfrow=c(1,2), mgp=c(1.75,0.75,0), mar=c(3,3,1,1))
spider_glmX = manyglm(spiderAbund~.,data=spiderX)
...
spiderPA_glmInt = manyglm(spiderPA~1,data=spiderX, family="cloglog")
...
par(mfrow=c(1,2), mgp=c(1.75,0.75,0), mar=c(3,3,1,1))
spiderPA_glmX = manyglm(spiderPA~.,data=spiderX, family="cloglog")
...
sum( abs(ord_spiderPAX$sigma)) ) - ncol(spider$PA)
c(absCor, 1-absCor[2]/absCor[1])
```
...
data(aviurba)
abund=aviurba$fau
for(iVar in 1:ncol(abund))
  abund[,iVar]=factor(abund[,iVar])
library(ordinal)
ft_birdsInt=manyany("clm", abund, abund~1, family="ordinal", data=aviurba$mil)
```
...
```{r ex17.3X, fig.width=8, fig.height=4}
par(mfrow=c(1,2), mgp=c(1.75,0.75,0), mar=c(3,3,1,1))
ft_birdsX=manyany("clm", abund, abund~fields, family="ordinal", data=aviurba$mil)
...
par(mfrow=c(1,2),mgp=c(1.75,0.75,0),mar=c(3,3,1,1))
spider_glmSoil = manyglm(spiderAbund~soil.dry,data=spiderX)
