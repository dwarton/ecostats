...
To get this working we currently need to store offset, pre-transformed, in workspace :( 
```{r code14.2, fig.width=9,fig.height=3.3}
logPit=log(reveg$pitfalls)
par(mfrow=c(1,3),mar=c(3,3,2,1),mgp=c(1.75,0.75,0))
ft_reveg=manyglm(abundMV~treatment,offset=logPit,family="negative.binomial", data=reveg)
plotenvelope(ft_reveg, which=1:3)
```

## Code Box 14.3: Checking mean-variance assumptions for a Poisson
revegetation model
```{r code14.3, fig.width=9, fig.height=3.3}
ft_revegP=manyglm(abundMV~treatment, offset=logPit, family="poisson", data=reveg)
...
Things generally look OK, but there is a weird downward kink on the scale-location plot, weakly suggestive of under-dispersion. This could be a weird artifact though so let's repeat using `"refit"`, but with a smaller value of `n.sim` so it doesn't take too long:
```{r ex14.6refit, fig.width=9, fig.height=3.3, warning=FALSE}
par(mfrow=c(1,3),mar=c(3,3,1,1),mgp=c(1.75,0.75,0))
plotenvelope(ft_wind, which=1:3, n.sim=39)
```

So the weird kink in the scale-location plot happened in simulated data too, so it is nothing really to worry about. It seems to be an artifact, probably because of overfitting of the data by including a `Station` term in the model means that there is a parameter for every two observations in the data!. (If a random effect were used instead, this effect would have been weaker. But it isn't really a problem)
...
habOrd = counts = round(seaweed[,6:21]*seaweed$Wmass)
 habOrd[counts>0 & counts<10] = 1
habOrd[counts>=10] = 2
library(ordinal)
for(iVar in 1:dim(habOrd)[2]) #clm needs each response to be a factor
  habOrd[,iVar]=factor(habOrd[,iVar],levels=c("0","1","2"))
summary(habOrd) # Amphipods are all "2" which would return an error in clm
habOrd=habOrd[,-1] #remove Amphipods
manyOrd=manyany("clm",habOrd,abund~Dist*Time*Size,data=seaweed)
manyOrdNull=manyany("clm",habOrd,abund~Time*Size,data=seaweed)
