...
```{r seaweed interplot}
 ...
 by.DistTime = group_by(seaweed,Dist,Time)
 distTimeMeans = summarise(by.DistTime, logTotal=mean(log(Total)))
 distTimeMeans
 library(ggplot2)
...
```{r snowmelt}
 ...
 ft_snow = lm(flow~elev+snow, data=snowmelt)
...
```{r seaweed anova3}
ft_seaweed3 = lm(logTot~Size*Time*Dist,data=seaweed)
